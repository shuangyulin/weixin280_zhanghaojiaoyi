<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="卖家账号" prop="maijiazhanghao">
          <el-select :disabled="ro.maijiazhanghao" @change="maijiazhanghaoChange" v-model="ruleForm.maijiazhanghao" placeholder="请选择卖家账号">
            <el-option
                v-for="(item,index) in maijiazhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.maijiazhanghao" label="卖家账号" prop="maijiazhanghao">
...
export default {
  ...
  methods: {
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.maijiazhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    maijiazhanghaoChange () {
      this.$http({
        url: `follow/maijia/maijiazhanghao?columnValue=`+ this.ruleForm.maijiazhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.maijiaxingming){
                this.ruleForm.maijiaxingming = data.data.maijiaxingming
              }
        } else {
          this.$message.error(data.msg);
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `maijiapingjia/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.maijiapingjiaCrossAddOrUpdateFlag = false;
						 this.parent.search();
